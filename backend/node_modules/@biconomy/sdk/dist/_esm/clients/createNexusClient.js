...
export async function createNexusClient(parameters) {
    const { client: client_, chain = parameters.chain ?? client_?.chain, signer, index = 0n, key = "nexus client", name = "Nexus Client", module, factoryAddress = k1ValidatorFactoryAddress, k1ValidatorAddress = k1ValidatorAddress_, bundlerTransport, transport, accountAddress, paymasterContext, attesters, attesterThreshold, ...bundlerConfig } = parameters;
    if (!chain)
