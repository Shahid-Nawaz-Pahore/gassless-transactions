import { MOCK_ATTESTER_ADDRESS, SMART_SESSIONS_ADDRESS, findTrustedAttesters, getSpendingLimitsPolicy, getSudoPolicy, getTrustAttestersAction, getUsageLimitPolicy, getValueLimitPolicy } from "@rhinestone/module-sdk";
...
import { SIMPLE_SESSION_VALIDATOR_ADDRESS } from "../../../constants/index.js";
import { SmartSessionAbi } from "../../../constants/abi/SmartSessionAbi.js";
import { abiToPoliciesInfo, applyDefaults, createActionConfig, createActionData, generateSalt, getPermissionId, toTimeRangePolicy, toUniversalActionPolicy } from "../Helpers.js";
/**
...
export const getPermissionAction = async ({ chainId, sessionRequestedInfo, client }) => {
    ...
    const resolvedPolicyInfo2ActionData = (actionPolicyInfo) => {
        ...
        if (actionPolicyInfo.tokenLimits?.length) {
            const spendingLimitPolicy = getSpendingLimitsPolicy(actionPolicyInfo.tokenLimits);
            policyData.push(spendingLimitPolicy);
        }
        // Create ActionData
        const actionPolicy = createActionData(actionPolicyInfo.contractAddress, actionPolicyInfo.functionSelector, policyData);
        return actionPolicy;
    };
...
export async function grantPermission(client, parameters) {
    ...
    if ("action" in actionResponse) {
        const userOpHash = (await getAction(client, sendUserOperation, "sendUserOperation")({ calls, maxFeePerGas, maxPriorityFeePerGas, nonce, account }));
        return {
